...
namespace AccountingPlusEA.Mfp
{
    ...
    public partial class JobList : ApplicationBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string accessRightsOnLogin = string.Empty;
            ...
            if (deviceModel == Constants.DEVICE_MODEL_DEFAULT)  //|| deviceModel == "Wide-XGA"
            {
            ...
            if (!IsPostBack)
            {
                ...
                accessRightsOnLogin = DataManagerDevice.ProviderDevice.ApplicationSettings.ProvideSetting("Access Rights For Login");
                if (accessRightsOnLogin.ToLower() == "disable")
                {
                ...
                {
                    //check Access rights for login
                    bool isLoginAccessAllowed = DataManagerDevice.ProviderDevice.Users.GetAccessRightsLogin(Session["UserSystemID"] as string, deviceIpAddress);

                    if (!isLoginAccessAllowed)
                    {
                        Response.Redirect("AccessDenied.aspx?aid=0");
                    }
                }
            ...
            if (!string.IsNullOrEmpty(applicationtype))
            {
                ...
                {
                    TableCellLimits.Visible = false;
                    TableCellBalance.Visible = true;
                    TableCellDevicemode.Visible = false;
                }
                else
                {
                    TableCellLimits.Visible = true;
                    TableCellBalance.Visible = false;
                    TableCellDevicemode.Visible = true;
                }
            }

            LabelCard.Text = Session["SelfCardId"] as string;
        ...
        protected void ImageButtonLimits_Click(object sender, EventArgs e)
        {
            Response.Redirect("LimitBalance.aspx?BtnLimit=true");
            //Response.Redirect("SelectCostCenter.aspx?BtnLimit=true");
            //Response.Redirect("LimitBalance.aspx");
        }

        protected void ImageButtonBalance_Click(object sender, EventArgs e)
        {
            Response.Redirect("CopyScan.aspx");
        }
        ...
        protected void LinkButtonLogOff_Click(object sender, EventArgs e)
        {
            ...
            //if (osaICCard)
            //{               
            DivJoblistredirect.Attributes.Add("style", "display:inline");
            PageLoadingID.Attributes.Add("style", "display:none");
            PageShowingID.Visible = false;
        ...
        private void LocalizeThisPage()
        {
            ...
            LabelLimits.Text = "Limits";//localizedResources["L_LIMITS"].ToString();
        ...
        {
            AddHorizantalLine();
            string jobUserID = string.Empty;
            if (Session["UserID"].ToString() != null)
            {
                jobUserID = Session["UserID"].ToString();
            }
            //string sqlQuery = string.Format("select * from T_PRINT_JOBS where USER_ID = '" + jobUserID + "' and JOB_PRINT_RELEASED = 'false' and datediff(ss, REC_DATE, getdate()) > 30");
            //DataSet dsJobsinQueue = new DataSet();
            //using (DataManagerDevice.Database database = new DataManagerDevice.Database())
            //{
            //    dsJobsinQueue = database.ExecuteDataSet(database.GetSqlStringCommand(sqlQuery));
            //}
            ...
            if (ApplicationHelper.IsServiceLive())
            {
                ...
                if (dtPrintJobsOriginal != null && rowsCount > 0)
                {
                    ...
                    for (int jobs = 0; jobs < rowCount; jobs++)
                    {
                        ...
                        drComman = drJobList;
                        rowCountJobs = jobs;
                        if (!string.IsNullOrEmpty(drComman[rowCountJobs]["IS_EMAIL"].ToString()))
                        {
                        ...
                        else
                        {
                            ...
                            // bool isjobIDExists = false;
                            //dsJobsinQueue.Tables[0].AsEnumerable().Any(row => jobID == row.Field<String>("JOB_ID"));
                    ...
                    if (!string.IsNullOrEmpty(onNoJobs))
                    {
                        if (onNoJobs != "Display Job List" && pageFrom != "Status")
                        {
                            if (!IsPostBack)
                            {
                                //MoveToDeviceMode();
                                Session["IsRedirectToDeviceMode"] = true;
                                Response.Redirect("../Mfp/SelectCostCenter.aspx");
                            }
                        }
                    }
                }
        ...
        private void MoveToDeviceMode()
        {
            ...
            {
                string auditorFailureMessage = ex.InnerException.ToString();
                string suggestionMessage = "UserId: "+ Convert.ToString(Session["UserID"] );
                LogManager.RecordMessage( Request.Params["REMOTE_ADDR"].ToString(), AUDITORSOURCE, LogManager.MessageType.Exception, auditorFailureMessage, suggestionMessage, ex.Message, ex.StackTrace);
            }
        }
        ...
        /// </summary>
        private void PrintAndDelete(string printType)
        {
            try
            {
                string selectedFiles = Request.Form["__SelectedFiles"] as string;
            ...
            try
            {
                if (dsDevices != null && dsDevices.Tables.Count > 0)
                {
                    ...
                    if (drDeviceDetails != null && drDeviceDetails.Length > 0)
                    {
                        ...
                        printSettingType = drDeviceDetails[0]["MFP_PRINT_TYPE"].ToString();
                        tcpPort = drDeviceDetails[0]["MFP_DIR_PRINT_PORT"].ToString();
        ...
        /// </remarks>
        private void PrintJobs(string PrintType)
        {
            Session["NewPrintSettings"] = null;
            string selectedFiles = Request.Form["__SelectedFiles"] as string;
            string printalljobs = DataManagerDevice.ProviderDevice.ApplicationSettings.ProvideSetting("Action for Print all jobs on login");
            ...
            if (!string.IsNullOrEmpty(applicationtype))
            {
                ...
                {
                    Response.Redirect("SelectCostCenter.aspx?pty=" + PrintType + "", false);
                }
            }
        }
