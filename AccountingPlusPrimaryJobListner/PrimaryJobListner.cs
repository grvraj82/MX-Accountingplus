...
{
    public partial class PrimaryJobListner : ServiceBase
    {
        public PrimaryJobListner()
        {
            InitializeComponent();
        }

        internal static string AUDITORSOURCE = "Primary Print Job Listner";
        internal static string SERVICE_NAME = "AccountingPlusPrimaryJobListner";
        ...
        {
            JobListner.JobProcessor.RecordServiceTimings("AccountingPlusPrimaryJobListner", listenerPort, "start");
            Task tskPrintJobListner = Task.Factory.StartNew(StartListiningPrintJob);
            //StartListiningPrintJob();
            //Task tskmonitorAccountingServices = Task.Factory.StartNew(InitializeTimer);
        ...
        {
            JobListner.JobProcessor.RecordServiceTimings("AccountingPlusPrimaryJobListner", listenerPort, "stop");
        }
        ...
        private static void StartListiningPrintJob()
        {
            ...
            catch (Exception ex)
            {
                ...
                LogManager.RecordMessage(AUDITORSOURCE, "StartListiningPrintJob", LogManager.MessageType.CriticalError, ex.Message, null, ex.Message, ex.StackTrace);
            }
        }
