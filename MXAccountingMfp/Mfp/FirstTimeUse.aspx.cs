        ...
        private void RedirectToJobListPage()
        {
            ...
            if (dsCostCenters.Tables[0] != null && dsCostCenters.Tables[0].Rows.Count != 0)
            {
                ...
                else
                {
                    ...
                    string deviceIpAddress = Request.Params["REMOTE_ADDR"].ToString();
                    bool isUserLoginAllowed = DataManagerDevice.ProviderDevice.Users.ProvideIsUserLoginAllowed(costCenter, deviceIpAddress);
                    ...
                    Response.Redirect("../Mfp/JobList.aspx?CC=" + costCenter + "", true);
                }
            }
            #endregion
        }
