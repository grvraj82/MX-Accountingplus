...
REM  Take HEAD clena update before batch creation
cvs "-q" "-x" "update" "-d" "-P" "-C" "."

REM BranchName

set "version=test"
 for /f "tokens=*" %%a in (Build.version) do (
...
for /f "tokens=3 delims=." %%i in ("%version%") do set "variable1=%%i" 
REM echo %variable1%

REM echo iSignae_%variable1%
...
cvs tag  -b AccountingPlus_%variable1%
